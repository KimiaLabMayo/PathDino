...
def create_gif_from_images(input_image, output_dir, attention_list):
    # Number of steps for the transition
    steps = 20

    for i in range(0, len(attention_list)):
        # read the activated attention map
        image2_path = os.path.join(output_dir, "attn-head" + str(i) + ".png")

        # Generate cross-fade images
        images = cross_fade(input_image, image2_path, steps)
        ...
        images[0].save(os.path.join(output_dir, "attn-head" + str(i) + ".gif"), save_all=True, append_images=images[1:], loop=0, duration=100)
...
def generate_activation_maps(input_image, output_dir="./output", patch_size=16):
    ...
    create_gif_from_images(input_image, output_dir, attention_list)
    print("GIFs saved.")
    return attention_list
